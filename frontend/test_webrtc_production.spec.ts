...
test("Production WebRTC Certification", async ({ browser }) => {
  ...
  // Frontend (Vercel) may deploy faster than backend (Render), so FE may be newer — that is OK.
  ...
  if (feCommit !== "unknown" && feCommit !== backendCommit) {
    console.log(`[DRIFT INFO] Frontend (${feCommit.substring(0,8)}) != Backend (${backendCommit.substring(0,8)}) — frontend may be newer, continuing.`);
  }
